...
/*
 * ESP32forth v7.0.7.21c
 * Updated: Marc PETREMANN - 18 may 2026
 */
...
// Force le compilateur à garder les noms lisibles pour le framework Arduino
extern "C" {
    void setup();
    void loop();
}
...
// #  include <NetworkInterface.h> // Optionnel mais sécurisant en 3.3.7
...
# define OPTIONAL_WIFI_SUPPORT \
  ...
  XV(WiFi, "WiFi.disconnect", WIFI_DISCONNECT, WiFi.disconnect(true)) \
...
/* WiFi IPv6 - Basé sur NetworkInterface.h (Core 3.x) */ \
/* WiFi IPv6 - Version compatible Core 3.3.7 */ \
/*  XV(WiFi, "WiFi.enableIpV6", WIFI_ENABLE_IPV6, PUSH (cell_t)WiFi.enableIpV6()) \
  XV(WiFi, "WiFi.localIPv6", WIFI_LINK_LOCAL_IPV6, \
     static String s1; s1 = WiFi.localIPv6().toString(); \
     PUSH (cell_t)s1.c_str(); PUSH (cell_t)s1.length()) \
  XV(WiFi, "WiFi.globalIPv6", WIFI_GLOBAL_IPV6, \
     static String s2; s2 = WiFi.globalIPv6().toString(); \
     PUSH (cell_t)s2.c_str(); PUSH (cell_t)s2.length()) \ */ \
/* WiFi scan */ \
